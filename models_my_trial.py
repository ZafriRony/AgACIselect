...
def fit_predict(X, Y, alpha, methods, params_methods, basemodel, params_basemodel, train_size):
    ...
    # initialize parameters for 'ACP' and 'ACP_selection'
    # ACP_selection is the same as ACP but alpha_t is only updated and CI is provided only if the lower bound > 47.
    if 'ACP' in methods or 'ACP_selection' in methods:
        alpha_t = alpha
            ...
            for idm, method in enumerate(methods):
                ...
                if method == "Gaussian":
                    ...
                    if(alpha_t >= 1): # => 1-alpha_t <= 0 => predict empty set
                        ...
                        err = 1-float((y_lower_i <= Y[train_size+i]) & (Y[train_size+i] <= y_upper_i))
                    ##################################################################################################
                    # ACP_selection: Same as ACP but alpha_t is only updated if the lower bound > 47.
                    # This implements selective updating to adjust for conditional coverage—predictions are always shown,
                    # but the calibration only reacts to "selected" observations.
                    # ONLY IF LB>47 alpha_t will be updated
                    selected = y_lower_i > 47
                    if selected:
                        alpha_t = alpha_t + gamma * (alpha - err)
                        y_lower_i, y_upper_i = y_pred-window, y_pred+window
                    else:
                        y_lower_i, y_upper_i = np.nan, np.nan  # Don't produce interval if not selected
...
def fit_predict_ACPs(X, Y, alpha, tab_gamma, basemodel, params_basemodel, train_size, method='ACP'):
...
#####
    #if method == 'ACP_selection':
        # ACP_selection is the same as ACP but alpha_t is only updated and CI is provided only if the lower bound > 47.
        # This implements selective updating to adjust for conditional coverage—predictions are always shown,
        # but the calibration only reacts to "selected" observations.
        # ONLY IF LB>47 alpha_t will be updated
    #    selected = np.zeros((len(tab_gamma),test_size))
####
    ...
    for i in range(test_size):
        ...
        for idg, gamma in enumerate(tab_gamma):
            ...
            if method == 'ACP_selection':
                ...
                selected = y_lower_i > 47
                if selected:
                    tab_alpha_t[idg,i] = alpha_t + gamma*(alpha-err)
                    y_lower_i, y_upper_i = y_pred-window, y_pred+window
                else:
                    tab_alpha_t[idg,i] = alpha_t #alpha_t is not updated
                    y_lower_i, y_upper_i = np.nan, np.nan # Don't produce interval if not selected
        ...
        # but the calibration only reacts to "selected" observations.
        # ONLY IF LB>47 alpha_t will be updated
    #    return y_lowers, y_uppers, tab_alpha_t, gammas, selected
    #else:
